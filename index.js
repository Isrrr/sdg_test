...
function calculateLeaderboardPlaces(users, minScores) {
  // code here
  let lastPushedPlace = 0;

  return users.map((user, index) => {
    if (user.score >= minScores.firstPlaceMinScore) {
      lastPushedPlace = 1;
      return { userId: user.userId, place: 1 };
    } else if (user.score >= minScores.secondPlaceMinScore) {
      lastPushedPlace = 2;
      return { userId: user.userId, place: 2 };
    } else if (user.score >= minScores.thirdPlaceMinScore) {
      lastPushedPlace = 3;
      return { userId: user.userId, place: 3 };
    } else {
      return { userId: user.userId, place: index + (4 - lastPushedPlace) };
    }
  });
}
